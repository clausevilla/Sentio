...
def get_recommendations (prediction, confidence, anxiety_level):
    ...
    if confidence > 0.70:             # high confidence results
        if prediction == 'normal':
            recommendations.append("You seem to present an undoubtedly healthy mental state. Remember to maintain regular sleep, nutrition, and physical activity. Check in with yourself regularly about emotional wellbeing, and do not hesitate to reach out for help in case of need.")
        elif prediction == 'stress':
            recommendations.append("You seem to present strong indicators of stress, try to limit caffeine and consider mindfulness meditation for 10 minutes daily. Delegate tasks where possible and practice setting boundaries, focusing on one task at a time can make a big difference.")
        ...
        if prediction == 'normal':
            recommendations.append("You seem to have a healthy mental state. Try to maintain regular sleep, nutrition, and physical activity. Check in with yourself regularly about emotional wellbeing, and do not hesitate to reach out for help in case of need.")
        elif prediction == 'stress':
            recommendations.append("You seem to present medium indicators of stress, try to limit caffeine and consider mindfulness meditation for 10 minutes daily. Delegate tasks where possible and practice setting boundaries, focusing on one task at a time can make a big difference.")
        elif prediction == 'suicidal':
        ...
        if prediction == 'suicidal':
            ...
            recommendations.append("Check in with yourself regularly about emotional wellbeing. In case you feel symptoms of depression, do not hesitate to reach out for professional help.")
        elif prediction == 'stress':
            recommendations.append("In case you feel stressed, try to limit caffeine and consider mindfulness meditation for 10 minutes daily. Delegate tasks where possible and practice setting boundaries, focusing on one task at a time can make a big difference.")

        else:
            recommendations.append("Your mental state seems normal. Ensure to maintain regular sleep, nutrition, and physical activity. Check in with yourself regularly about emotional wellbeing, and do not hesitate to reach out for help in case of need.")

    if anxiety_level >= 50:
        recommendations.append("Additionally, your text presents a high anxiety level, consider practicing daily grounding techniques (5-4-3-2-1 method). Try to take a step back and think long-term; it might feel less overwhelming. Remember to always prioritize your mental health.")
...
def calculate_metrics(text: str):
    ...
    anxiety_score = (ellipsis_count * 50) + (period_count * 20) + (word_density * 25) + (capital_count * 0.5)

    # Emotional intensity: Exclamations > Questions (strong emotion > curiosity)
    emotional_score = (exclamation_count * 40) + (question_count * 30) + (capital_count * 20) + (word_density * 10)
